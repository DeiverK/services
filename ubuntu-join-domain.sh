...
function system_update {
	[[ $(apt -y update) ]] || { echo "There is a problem updating the system, stopping the process for manual check"; exit; }
}
...
# Installs the required packages in Ubuntu
	[[ $(echo -ne '\n' | apt -y install realmd libnss-sss libpam-sss sssd sssd-tools adcli samba-common-bin oddjob oddjob-mkhomedir packagekit krb5-user) ]] || exit
...
# joins the VM into the domain
	[[ $(realm list | grep configured) ]] || realm join --verbose ${upperdomain} -U "${DOMAINADMIN}@${upperdomain}"
}
...
function configure_sudoers {
	if [[ ${AD_USER_SUDOERS} != "" ]]
	then
		[[ $(grep ${AD_USER_SUDOERS} /etc/sudoers) ]] || echo "${AD_USER_SUDOERS}@${loweerdomain} ALL=(ALL:ALL) ALL" >> /etc/sudoers
	fi
	
	if [[ ${AD_GROUP_SUDOERS} != "" ]]
	then
		[[ $(grep ${AD_GROUP_SUDOERS} /etc/sudoers) ]] || echo "%${AD_GROUP_SUDOERS}@${loweerdomain} ALL=(ALL:ALL) ALL" >> /etc/sudoers
	fi
	
	echo "sudoers was configured"
}
